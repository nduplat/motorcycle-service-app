...
export class QueueManagementComponent implements OnInit, OnDestroy {
  ...
  private readonly POLLING_INTERVALS = {
    active: 5000,    // 5 seconds when queue is active
    idle: 30000,     // 30 seconds when idle
    critical: 2000   // 2 seconds when critical alerts
  };
  private lastActivityTime = Date.now();
  private pollingInterval = signal(this.POLLING_INTERVALS.active);
  ...
  // Intelligent polling methods
  private startIntelligentPolling(): void {
    this.stopIntelligentPolling(); // Clean up any existing subscription
  ...
  private async pollQueueData(): Promise<void> {
    try {
      // Refresh queue data
      await this.queueService.refreshQueueData();

      // Update activity-based polling interval
      this.updatePollingInterval();

      // Recalculate wait times periodically
      if (Date.now() - this.lastActivityTime > 60000) { // Every minute
        await this.calculateWaitTimes();
      }
    } catch (error) {
  ...
  private updatePollingInterval(): void {
    ...
    const hasCriticalAlerts = this.alerts().some(alert => alert.severity === 'critical');
    const queueLength = this.waitingCount() + this.calledCount();

    let newInterval: number;

    if (hasCriticalAlerts) {
      newInterval = this.POLLING_INTERVALS.critical;
    } else if (queueLength > 0 || timeSinceActivity < 300000) { // Active in last 5 minutes
      newInterval = this.POLLING_INTERVALS.active;
    } else {
      newInterval = this.POLLING_INTERVALS.idle;
    }
  ...
  // Override user action methods to track activity
  async callNext() {
    this.updateActivity();

  ...
  createWorkOrder(workOrder: Omit<WorkOrder, 'id' | 'createdAt'>): Observable<WorkOrder> {
    return from(new Promise<WorkOrder>(async (resolve, reject) => {
        try {
            ...
            // For heavy operations, use background processing
            if (workOrder.services && workOrder.services.length > 2) {
                ...
                const job = await this.jobQueueService.enqueueJob(
                    ...
                    {
                        priority: workOrder.priority === 'urgent' ? JobPriority.URGENT : JobPriority.HIGH,
                        createdBy: currentUser.id
                    }
                );

                // Return a placeholder work order that will be updated when job completes
                ...
                resolve(placeholderWorkOrder);
                return;
            }

            // For simple work orders, create immediately
  ...
  updateWorkOrder(updatedWO: WorkOrder): Observable<WorkOrder> {
    return from(new Promise<WorkOrder>(async (resolve, reject) => {
      try {
        ...
        // Invalidate specific work order cache
  ...
  createWorkOrderFromQueueEntry(entry: QueueEntry, createdBy: string): Observable<WorkOrder> {
    return from(new Promise<WorkOrder>(async (resolve, reject) => {
      try {
        ...
        // Invalidate specific work order cache
        this.cacheService.invalidateByEntity('work-order', newWorkOrder.id).catch(err =>
          console.error('Work order creation cache invalidation error:', err)
        );
